...
procedure {:inline 1} write(la: linear_address, d: data)
modifies arbitrary_write_count, mem_reg, mem_secs, mem_tcs; 
{
  ...
  //FIXME: Nov 9, 2014
  assume pa != abort_page; //executions that proceed must meet this criterion
  if (pa != abort_page) {
    call unchecked_write_reg(pa, d);
  }
}

procedure {:inline 1} read(la: linear_address) returns (result: data)
{
  ...
  //FIXME: Nov 9, 2014
  assume pa != abort_page; //executions that proceed must meet this criterion
  if (pa != abort_page) { result := mem_reg[pa]; } 
  else { result := abort_data; }
}

procedure {:inline 1} switch_thread (lp : lp_id)
modifies curr_lp;
{
  curr_lp := lp;
}

procedure {:inline 1} skip() { /* do nothing */ }
...
procedure {:inline 1} update_page_table(l: linear_address, p: physical_address)
modifies page_table;
{
  page_table[l] := p;
}
...
//********************* Properties *********************
function secs_valid(epcm : [physical_address] epcm_entry_ty, mem_secs : [physical_address] secs_ty) : bool; 
axiom (forall epcm : [physical_address] epcm_entry_ty, mem_secs : [physical_address] secs_ty ::
       {secs_valid(epcm, mem_secs)}
       secs_valid(epcm, mem_secs) <==> (forall pa : physical_address ::
                                        (is_epc_address(pa) && Epcm_valid(epcm[pa]) && Epcm_pt(epcm[pa]) != pt_secs) ==> 
                                        (is_epc_address(Epcm_enclavesecs(epcm[pa])) && 
                                         Epcm_valid(epcm[Epcm_enclavesecs(epcm[pa])]) && 
                                         Epcm_pt(epcm[Epcm_enclavesecs(epcm[pa])]) == pt_secs)
                                       )); 

function lp_secs_inv(lp_state: [lp_id] lp_state_ty, epcm : [physical_address] epcm_entry_ty) : bool;
axiom (forall lp_state : [lp_id] lp_state_ty, epcm : [physical_address] epcm_entry_ty :: 
      {lp_secs_inv(lp_state, epcm)}
      lp_secs_inv(lp_state, epcm) <==> (forall lp: lp_id ::
...
function lp_tcs_valid(lp_state: [lp_id] lp_state_ty, epcm : [physical_address] epcm_entry_ty, mem_tcs : [physical_address] tcs_ty) : bool;
axiom (forall lp_state : [lp_id] lp_state_ty, epcm : [physical_address] epcm_entry_ty, mem_tcs : [physical_address] tcs_ty :: 
      {lp_tcs_valid(lp_state, epcm, mem_tcs)}
      lp_tcs_valid(lp_state, epcm, mem_tcs) <==> (forall lp: lp_id ::
                                                 (Lp_state_cr_enclave_mode(lp_state[lp]) ==>
