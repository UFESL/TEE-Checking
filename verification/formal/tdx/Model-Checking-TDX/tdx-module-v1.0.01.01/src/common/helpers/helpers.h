...
{
    // Moking private_hkid_min and hkind max

    uint32_t private_hkid_min_mock; 
    uint32_t private_hkid_max_mock;
    // return ((uint32_t)hkid >= get_global_data()->private_hkid_min &&
    //         (uint32_t)hkid <= get_global_data()->private_hkid_max);
    return ((uint32_t)hkid >= private_hkid_min_mock &&
            (uint32_t)hkid <= private_hkid_max_mock);
}
