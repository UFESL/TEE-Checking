...
api_error_type tdh_mng_key_freeid(uint64_t target_tdr_pa)
{
    ...
    // assert(global_data_ptr->kot.entries[curr_hkid].state == KOT_STATE_HKID_FLUSHED);
    ...
    assert((global_data_ptr->kot.entries[curr_hkid].state == KOT_STATE_HKID_FREE) && (tdr_ptr->management_fields.lifecycle_state == TD_TEARDOWN));
EXIT:
